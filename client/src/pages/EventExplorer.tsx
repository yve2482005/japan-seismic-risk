...
import { isCloseEpicenterDistance, isWithinJapanMapBounds, mapCoordinate, mapDistanceSegment, mapMarkerStyle } from "@/lib/earthquakeMap";
import { JAPAN_REGIONS, type JapanRegion, type SeismicEvent } from "@shared/seismic";
import { AlertTriangle, Ambulance, ArrowLeft, Crosshair, ExternalLink, Flame, Info, LocateFixed, MapPin, Search, ShieldAlert } from "lucide-react";
...
export default function EventExplorer() {
  ...
  const selectedDistanceKm = location && activeEvent ? approximateDistanceKm(location, { latitude: activeEvent.latitude, longitude: activeEvent.longitude }) : null;
  const selectedEventIsClose = isCloseEpicenterDistance(selectedDistanceKm);
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[1.42fr_0.58fr]">
          <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="relative mt-5 aspect-[1.16/1] overflow-hidden rounded-2xl border border-slate-200 bg-[radial-gradient(circle_at_70%_30%,#f9fdff_0%,#dceff8_38%,#c5e0ed_100%)]">
              ...
              {locationInMap && location && activeEvent && (() => { const from = mapCoordinate(location.latitude, location.longitude); const to = mapCoordinate(activeEvent.latitude, activeEvent.longitude); const segment = mapDistanceSegment(from, to); return <><div aria-hidden="true" className={`absolute z-[5] h-0 border-t-2 border-dashed ${selectedEventIsClose ? "border-[#b42318]" : "border-slate-900/70"}`} style={{ left: `${segment.left}%`, top: `${segment.top}%`, width: `${segment.length}%`, transform: `rotate(${segment.angle}deg)`, transformOrigin: "0 0" }} /><span className={`absolute z-[15] -translate-x-1/2 -translate-y-1/2 rounded-full px-2 py-1 text-[9px] font-black text-white shadow ${selectedEventIsClose ? "bg-[#b42318]" : "bg-slate-950"}`} style={{ left: `${segment.labelLeft}%`, top: `${segment.labelTop}%` }}>≈ {selectedDistanceKm} km</span></>; })()}
          ...
          <EventDetail event={activeEvent} distanceKm={selectedDistanceKm} />
        </section>
...
function EventDetail({ event, distanceKm }: { event: SeismicEvent | null; distanceKm: number | null }) {
  const closeDistance = isCloseEpicenterDistance(distanceKm);
  return <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-[10px] font-extrabold tracking-[0.18em] text-slate-500">SELECTED EVENT</p>{event ? <div className="mt-3"><div className="flex items-start justify-between gap-3"><div><h2 className="text-3xl font-black">M{event.magnitude.toFixed(1)}</h2><p className="mt-1 font-bold">{event.locality}</p></div><span className="rounded-full bg-[#edf7fc] px-2.5 py-1 text-[10px] font-bold text-slate-700">USGS live</span></div>{distanceKm !== null && <div className={`mt-4 rounded-xl border px-3 py-3 ${closeDistance ? "border-[#ef9389] bg-[#fff1ef] text-[#9f1d13]" : "border-[#bdd5c6] bg-[#f2fbf5]"}`}><p className="text-[10px] font-extrabold tracking-[0.14em]">FROM CURRENT LOCATION</p><p className="mt-1 text-lg font-black">Approximately {distanceKm.toLocaleString()} km</p>{closeDistance && <p className="mt-1 text-xs font-black">Within 100 km — check official local information if needed.</p>}<p className={`mt-1 text-xs leading-5 ${closeDistance ? "text-[#9f1d13]" : "text-slate-600"}`}>Epicenter သို့ တန်းဖြောင့်အကွာအဝေးသာဖြစ်ပြီး လမ်းခရီးအကွာအဝေး သို့မဟုတ် official warning area မဟုတ်ပါ။</p></div>}{closeDistance && <CloseDistanceSafety />}<p className="mt-4 rounded-xl bg-slate-50 px-3 py-2 text-xs leading-5 text-slate-600">Map marker သည် USGS က ထုတ်ပေးသော epicenter coordinate ကို approximate map ပေါ်တွင်ထားခြင်းဖြစ်သည်။ တရားဝင် emergency warning area မဟုတ်ပါ။</p><dl className="mt-4 divide-y divide-slate-100 text-sm"><Detail label="Region" value={event.region} /><Detail label="Depth" value={event.depthKm === null ? "Unknown" : `${event.depthKm.toFixed(1)} km`} /><Detail label="Time" value={`${localTime(event.originTimeUtc)} JST`} /><Detail label="Source" value="USGS public CSV" /></dl><a href={event.sourceUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-slate-700 underline underline-offset-4"><MapPin size={14} />Open source record</a></div> : <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600"><Info className="mb-2" size={18} />Marker တစ်ခုကိုနှိပ်ပြီး event details ကိုကြည့်ပါ။ User location သည် optional ဖြစ်ပြီး page ကိုပိတ်လျှင် မသိမ်းထားပါ။</div>}</article>;
}

function CloseDistanceSafety() {
  const [ambulanceFire, police] = CLOSE_DISTANCE_SAFETY.contacts;
  return <section aria-labelledby="close-distance-safety-title" className="mt-4 rounded-2xl border border-[#ef9389] bg-[#fff7f5] p-4 text-slate-900"><div className="flex items-start gap-2"><AlertTriangle className="mt-0.5 shrink-0 text-[#b42318]" size={18} /><div><p className="text-[10px] font-extrabold tracking-[0.14em] text-[#9f1d13]">CLOSE-DISTANCE SAFETY REMINDER</p><h3 id="close-distance-safety-title" className="mt-1 text-base font-black">အရေးပေါ်အတွက် ဆက်သွယ်ရန်နှင့် ဘေးကင်းရေး</h3><p className="mt-1 text-xs leading-5 text-slate-700">Distance 100 km အတွင်းဟု ပြသခြင်းသည် official warning သို့မဟုတ် evacuation order မဟုတ်ပါ။ အရေးပေါ်တကယ်ဖြစ်လျှင်သာ အောက်ပါနံပါတ်များကိုခေါ်ပါ။</p></div></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><a href={ambulanceFire.href} className="flex items-center gap-3 rounded-xl bg-[#b42318] px-3 py-3 text-white shadow-sm hover:bg-[#901f17]"><Ambulance size={18} /><span><span className="block text-lg font-black">{ambulanceFire.number}</span><span className="block text-[10px] font-bold">{ambulanceFire.label}</span></span></a><a href={police.href} className="flex items-center gap-3 rounded-xl border border-[#ef9389] bg-white px-3 py-3 text-[#9f1d13] hover:bg-[#fff1ef]"><ShieldAlert size={18} /><span><span className="block text-lg font-black">{police.number}</span><span className="block text-[10px] font-bold">{police.label}</span></span></a></div><div className="mt-4 space-y-2 rounded-xl bg-white/80 p-3 text-xs leading-5 text-slate-700"><p className="flex gap-2"><ShieldAlert className="mt-0.5 shrink-0 text-[#b42318]" size={15} /><span>Calm နေပြီး သင့်အနီးအနားရှိ အန္တရာယ်များမှ ကိုယ့်ကိုကာကွယ်ကာ staff နှင့် local authority ညွှန်ကြားချက်များကိုလိုက်နာပါ။</span></p><p className="flex gap-2"><Flame className="mt-0.5 shrink-0 text-[#b42318]" size={15} /><span>Building အတွင်းဖြစ်လျှင် လုံခြုံမှသာ အပြင်ထွက်ပါ; မီးအန္တရာယ်ကိုလည်း သတိပြုပါ။</span></p><p className="flex gap-2"><MapPin className="mt-0.5 shrink-0 text-[#b42318]" size={15} /><span>ကမ်းရိုးတန်းတွင် အားပြင်း/ကြာရှည်တုန်ခါမှုရှိလျှင် official tsunami information ကိုစစ်ပြီး higher ground သို့သွားပါ။</span></p></div><div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs font-bold"><a href={CLOSE_DISTANCE_SAFETY.resources.safetyTips} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#9f1d13] underline underline-offset-4">Japan Tourism Agency Safety Tips <ExternalLink size={12} /></a><a href={CLOSE_DISTANCE_SAFETY.resources.emergencyContacts} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#9f1d13] underline underline-offset-4">Emergency contact source <ExternalLink size={12} /></a></div></section>;
}
