...
import { isWithinJapanMapBounds, mapCoordinate, mapDistanceSegment, mapMarkerStyle } from "@/lib/earthquakeMap";
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[1.42fr_0.58fr]">
          <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="relative mt-5 aspect-[1.16/1] overflow-hidden rounded-2xl border border-slate-200 bg-[radial-gradient(circle_at_70%_30%,#f9fdff_0%,#dceff8_38%,#c5e0ed_100%)]">
              ...
              {locationInMap && location && activeEvent && (() => { const from = mapCoordinate(location.latitude, location.longitude); const to = mapCoordinate(activeEvent.latitude, activeEvent.longitude); const segment = mapDistanceSegment(from, to); return <><div aria-hidden="true" className="absolute z-[5] h-0 border-t-2 border-dashed border-slate-900/70" style={{ left: `${segment.left}%`, top: `${segment.top}%`, width: `${segment.length}%`, transform: `rotate(${segment.angle}deg)`, transformOrigin: "0 0" }} /><span className="absolute z-[15] -translate-x-1/2 -translate-y-1/2 rounded-full bg-slate-950 px-2 py-1 text-[9px] font-black text-white shadow" style={{ left: `${segment.labelLeft}%`, top: `${segment.labelTop}%` }}>≈ {selectedDistanceKm} km</span></>; })()}
