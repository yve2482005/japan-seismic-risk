...
const fills = { LOW: "#c8dded", MODERATE: "#f7c7c2", ELEVATED: "#ef8e85", HIGH: "#dc5d54", UNAVAILABLE: "#e8edf0" } as const;
  ...
  return (
    <div className="map-shell" aria-label="Schematic interactive Japan regional activity map">
      <svg viewBox="0 0 360 410" className="h-auto w-full max-h-[420px]" role="img" aria-labelledby="map-title map-description">
        ...
        <desc id="map-description">A schematic regional map. Select a region to inspect its verified activity and model probability when a validated model is available.</desc>
        ...
        {regions.map(item => (
          <g
            ...
            role="button"
            tabIndex={0}
            aria-label={`${item.region}, ${item.risk}, ${item.probabilityM4_24h === null ? "model probability unavailable" : `${item.probabilityM4_24h.toFixed(1)} percent M4 plus probability in 24 hours`}`}
        ...
        <text x="14" y="399" fontSize="9" fill="#667085">Schematic view · circle = verified events in 24h</text>
      </svg>
